...
class AdivinaNumero:
    ...
    def validar_numero(self, numero):
        ListaValidos = list(filter(lambda x: x == numero, range(1, 101)))
    ...
    def jugar(self):
        ...
        adivino = False
        while self.intentos < self.max_intentos:  # ← indentación corregida
            try:
                intento = int(input(f"\nIntento {self.intentos + 1}: Ingresa un numero: "))

                if not self.validar_numero(intento):
                    print("El numero debe estar entre 1 y 100, intentalo de nuevo")
                    continue

                self.intentos += 1
                self.historial.append([self.intentos, intento])
